...
def list_funding_programs(
    ...
    - **organization**: Filter by organization (e.g., 'EU', 'État')
    ...
    if organization:
        query = query.filter(FundingProgram.organization.ilike(f"%{organization}%"))
...
def get_funding_program_filters(db: Session = Depends(get_db)):
    ...
    Returns distinct values for countries, organizations, themes, and funding types.
    """
    ...
    organizations = db.query(FundingProgram.organization).distinct().all()
    organizations = sorted([o[0] for o in organizations if o[0]])
...
def update_funding_program(
    ...
    update_fields = [
        'name', 'organization', 'country', 'eligible_themes', 'eligible_population_max',
